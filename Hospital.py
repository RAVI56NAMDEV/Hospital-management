...
class Hospital:
    def __init__(self,root):        
        self.root=root
        self.root.title("Hospital Management System By Ravi Kumar Namdev")
    ...
    def iPrescriptionData(self):
        if self.Nameoftablets.get() == "" or self.ref.get() == "" :
            ...
            conn = sql.connect(host="localhost",username="root",password="system",database="sqlprojectdata")
    ...
    def update(self):
        conn = sql.connect(host="localhost", username="root", password="system", database="sqlprojectdata")
        my_cursor = conn.cursor()
        my_cursor.execute("update hospital set Nameoftables=%s,Dose= %s,Numberoftablets= %s,Lot= %s,Issuedate=%s,Expdate=%s,Dailydose=%s,StorageAdvice= %s,nhsnumber=%s,patientname= %s,DOB= %s,PatientAddr=%s where ref= %s",(
                self.Nameoftablets.get(),
                self.Dose.get(),
                self.Numberoftablets.get(),
                self.Lot.get(),
